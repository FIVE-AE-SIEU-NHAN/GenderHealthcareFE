        ...
        <CardHeader className='relative p-0'>
          ...
          <div className='absolute top-10 left-10'>
            <Avatar className='border-semi-dark-blue h-32 w-32 border-6 bg-gray-200/70 shadow-lg'>
              <AvatarImage
                src={`https://api.dicebear.com/9.x/avataaars/svg?seed=${consultant.name}`}
                alt={consultant.name}
              />
              <AvatarFallback className='bg-muted text-4xl font-bold'>{getInitials(consultant.name)}</AvatarFallback>
            </Avatar>
          </div>
          <div className='flex justify-end p-4'>
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
              ...
              <DialogContent className='sm:max-w-[520px]'>
                ...
                <EditProfileForm currentUser={consultant} onSuccess={() => setIsEditDialogOpen(false)} />
              </DialogContent>
            </Dialog>
          </div>
        </CardHeader>
        <CardContent className='space-y-6 px-8 pt-6 pb-8'>
          ...
          <div className='grid grid-cols-1 gap-4 pt-2 md:grid-cols-2'>
            <InfoItem
              icon={<Phone />}
              label='Phone Number'
              value={consultant.phone_number || 'N/A'}
              iconColor='text-sky-500'
            />
            <InfoItem icon={<Cake />} label='Birthday' value={birthDate} iconColor='text-pink-500' />
            <InfoItem
              icon={
                consultant.gender === 'male' ? <Mars /> : consultant.gender === 'female' ? <Venus /> : <VenusAndMars />
              }
              label='Gender'
      ...
      <Card className='animate-fade-in-up w-full rounded-2xl pt-7 pb-7 shadow-lg [animation-delay:300ms] [animation-fill-mode:backwards]'>
        ...
        <CardContent className='grid grid-cols-1 gap-4 md:grid-cols-2'>
          <InfoItem
            ...
            value={formatSpecialization(consultant.specialization_1)}
            iconColor='text-teal-500'
          />
          <InfoItem
            icon={<Sparkles />}
            label='Secondary Specialization'
            value={formatSpecialization(consultant.specialization_2)}
            iconColor='text-amber-500'
          />
          <InfoItem
            icon={<ScrollText />}
            label='Certifications'
            value={consultant.certifications || 'Not Provided'}
