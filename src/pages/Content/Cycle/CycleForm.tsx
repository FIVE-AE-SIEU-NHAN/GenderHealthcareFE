...
function predictStatus(phase: string, data: StatusInput) {
  ...
  if (data.stress !== undefined && data.sleep_hours !== undefined && data.stress >= 4 && data.sleep_hours < 6) {
    notes.push("Căng thẳng cao và ngủ ít có thể ảnh hưởng chu kỳ.");
  }
  if (phase === "ovulation" && data.libido !== undefined && data.libido <= 2) {
    notes.push("Libido thấp trong giai đoạn rụng trứng, nên kiểm tra sức khỏe.");
  }
  if (phase === "luteal" && data.mood !== undefined && data.mood <= 2) {
    notes.push("Tâm trạng xấu trong PMS, có thể là dấu hiệu tiền kinh nguyệt.");
  }
  if (phase === "follicular" && data.energy !== undefined && data.energy <= 2) {
    notes.push("Thiếu năng lượng trong giai đoạn nang noãn, chú ý sức khỏe.");
  }
...
export default function AdminSafe() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      alert("Không thể lấy danh sách hồ sơ!");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      {/* ...HEADER & DASHBOARD ... (giữ như code cũ của bạn) */}
          ...
          <CardContent className="p-6">
            ...
            <div className="flex items-center space-x-6 mb-8">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Tìm kiếm bệnh nhân, ghi chú y tế hoặc mã hồ sơ..."
                  value={searchTerm}
            ...
            <div className="rounded-2xl border-2 border-blue-200/50 overflow-hidden shadow-lg bg-white/80">
              <Table>
                <TableHeader className="bg-gradient-to-r from-blue-600 to-blue-700">
                  <TableRow>
                    <TableHead className="text-white font-bold py-4">Mã hồ sơ</TableHead>
                    <TableHead className="text-white font-bold py-4">Bệnh nhân</TableHead>
                    <TableHead className="text-white font-bold py-4">Ngày kinh nguyệt</TableHead>
                    <TableHead className="text-white font-bold py-4">Chu kỳ</TableHead>
                    <TableHead className="text-white font-bold py-4">Thời gian</TableHead>
                    <TableHead className="text-white font-bold py-4">Ghi chú y tế</TableHead>
                    <TableHead className="text-white font-bold py-4">Theo dõi</TableHead>
                    <TableHead className="text-white font-bold py-4">Ngày khám</TableHead>
                    <TableHead className="text-white font-bold py-4 text-center">Thao tác</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    >
                      ...
                      <TableCell className="py-4">
                        <span className="px-3 py-1 bg-cyan-100 text-cyan-800 rounded-full text-sm font-semibold">
                          {record.cycle_length} ngày
                        </span>
                      </TableCell>
                      <TableCell className="py-4">
                        <span className="px-3 py-1 bg-indigo-100 text-indigo-800 rounded-full text-sm font-semibold">
                          {record.period_length} ngày
                        </span>
                      </TableCell>
        ...
        <DialogContent className="max-w-3xl border-2 border-cyan-200 shadow-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader className="border-b border-blue-100 pb-4">
            <DialogTitle className="flex items-center gap-3 text-xl">
              ...
              <div>
                <h3 className="text-xl font-bold text-blue-800">Tạo hồ sơ y tế mới</h3>
                <p className="text-sm text-blue-600 font-medium">Nhập thông tin bệnh nhân</p>
              </div>
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="last_period_date">Ngày kinh nguyệt đầu tiên *</Label>
              <Input
                id="last_period_date"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="cycle_length">Chu kỳ (ngày)</Label>
                <Input
                  id="cycle_length"
                  name="cycle_length"
                  type="number"
              ...
              <div>
                <Label htmlFor="period_length">Thời gian kinh (ngày)</Label>
                <Input
                  id="period_length"
                  name="period_length"
                  type="number"
            ...
            <div>
              <Label className="font-medium">Tâm trạng hôm nay</Label>
            ...
            {/* Libido */}
            <div>
              <Label className="font-medium">Libido hôm nay</Label>
              <div className="flex gap-2 mt-2">
            ...
            {/* Stress */}
            <div>
              <Label className="font-medium">Căng thẳng</Label>
              <div className="flex gap-2 mt-2">
            ...
            {/* Sleep hours */}
            <div>
              <Label className="font-medium">Giờ ngủ hôm qua</Label>
              <Input
                type="number"
                min="0"
                max="24"
            ...
            {/* Energy */}
            <div>
              <Label className="font-medium">Năng lượng</Label>
              <div className="flex gap-2 mt-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="remind_ovulation">Nhắc rụng trứng</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="remind_period"
                ...
                />
                <Label htmlFor="remind_period">Nhắc chu kỳ tiếp theo</Label>
              </div>
            </div>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-lg border-2 border-indigo-200 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit_last_period_date">Ngày kinh nguyệt đầu tiên *</Label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit_cycle_length">Chu kỳ (ngày)</Label>
                <Input
              ...
              <div>
                <Label htmlFor="edit_period_length">Thời gian kinh (ngày)</Label>
                <Input
          ...
          {selectedRecord && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Ngày kinh nguyệt</Label>
                  <p>{formatDate(selectedRecord.last_period_date)}</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Chu kỳ</Label>
                  <p>{selectedRecord.cycle_length} ngày</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Thời gian kinh</Label>
                  <p>{selectedRecord.period_length} ngày</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Nhắc nhở</Label>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {selectedRecord.mood !== undefined && (
                      <div className="bg-white p-3 rounded-lg shadow-md">
                        <span className="text-pink-600 font-medium">Tâm trạng:</span>
                    ...
                    {selectedRecord.sleep_hours !== undefined && (
                      <div className="bg-white p-3 rounded-lg shadow-md">
                        <span className="text-cyan-600 font-medium">Giờ ngủ:</span>
                        <div className="font-bold text-cyan-700">{selectedRecord.sleep_hours}h</div>
                      </div>
                    )}
                    {selectedRecord.energy !== undefined && (
                      <div className="bg-white p-3 rounded-lg shadow-md">
                        <span className="text-teal-600 font-medium">Năng lượng:</span>
              ...
              {prediction && (
                <div className="bg-pink-50 rounded-lg p-4 border border-pink-200 flex flex-col gap-3">
                  <div className="font-bold text-pink-700 mb-2">Kết quả dự đoán chu kỳ</div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-xs font-semibold text-gray-500">Ngày kinh tiếp theo</Label>
                      <div className="font-semibold text-cyan-800">{formatDate(prediction.next_period_date)}</div>
                    </div>
                    <div>
                      <Label className="text-xs font-semibold text-gray-500">Ngày rụng trứng</Label>
                      <div className="font-semibold text-blue-700">{formatDate(prediction.ovulation_date)}</div>
                    </div>
                    <div>
                      <Label className="text-xs font-semibold text-gray-500">Bắt đầu cửa sổ thụ thai</Label>
                      <div className="font-semibold text-green-700">{formatDate(prediction.fertile_window_start)}</div>
                    </div>
                    <div>
                      <Label className="text-xs font-semibold text-gray-500">Kết thúc cửa sổ thụ thai</Label>
                      <div className="font-semibold text-green-700">{formatDate(prediction.fertile_window_end)}</div>
                    </div>
                    <div>
                      <Label className="text-xs font-semibold text-gray-500">Khả năng mang thai</Label>
                      <div
                        className={
                          "font-bold " +
                      ...
                      >
                        {prediction.pregnancy_risk ? prediction.pregnancy_risk.replace("_", " ") : ""}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-lg border-2 border-red-200 shadow-2xl">
          <DialogHeader className="border-b border-red-100 pb-4">
            <DialogTitle className="flex items-center gap-3 text-xl text-red-700">
              ...
              <div>
                <h3 className="text-xl font-bold">Xác nhận xóa hồ sơ</h3>
                <p className="text-sm text-red-600 font-medium">Thao tác này không thể hoàn tác</p>
              </div>
            </DialogTitle>
            <DialogDescription className="text-slate-600 mt-2 text-base">
              Bạn có chắc chắn muốn xóa hồ sơ y tế này không? Tất cả dữ liệu liên quan sẽ bị mất vĩnh viễn.
            </DialogDescription>
          </DialogHeader>
          {selectedRecord && (
            <div className="space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-md p-4">
                <p className="text-sm font-medium">Bản ghi sẽ bị xóa:</p>
