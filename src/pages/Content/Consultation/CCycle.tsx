...
export default function AdminSafe() {
  ...
  const fetchPredictionForCycle = async (cycleId: string) => {
    const token = localStorage.getItem("access_token");
    const res = await axios.get(
      `http://localhost:3000/cycle-prediction/cycle/${cycleId}`,
  ...
  const fetchRecords = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const res = await axios.get("http://localhost:3000/cycle", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const openViewDialog = async (record: CycleRecord) => {
    ...
    setPrediction(null); // clear prediction cũ
    try {
      const prediction = await fetchPredictionForCycle(record.id);
