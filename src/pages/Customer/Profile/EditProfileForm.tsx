...
export function EditProfileForm({ currentUser, onSuccess }: EditProfileFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      date_of_birth: currentUser.date_of_birth ? new Date(currentUser.date_of_birth) : undefined
    }
  })
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      await updateProfileMutation.mutateAsync(changedValues as UpdateProfilePayload)
      onSuccess()
    } catch (error) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
        <Card className='animate-fade-in-up pt-4 pb-4 [animation-delay:100ms] [animation-fill-mode:backwards]'>
          <CardHeader>
            <CardTitle className='text-semi-dark-blue flex items-center gap-2 text-lg text-shadow-md'>
              <UserCircle className='text-semi-dark-blue h-6 w-6' />
              Personal Information
            </CardTitle>
            <CardDescription>Update your personal details. Click save when you are finished.</CardDescription>
          </CardHeader>
          <CardContent>
