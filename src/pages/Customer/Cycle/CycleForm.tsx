...
export default function AdminSafe() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
                        ...
                        <div className="flex items-center justify-center">
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem
                                onClick={() => setCalendarRecord(record)}
      ...
      <Dialog open={!!calendarRecord} onOpenChange={(v) => !v && setCalendarRecord(null)}>
        <DialogContent className="max-w-4xl p-4">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              <CalendarDays className="h-6 w-6 text-rose-500" />
              <span>Xem lịch chu kỳ của {calendarRecord?.user_name}</span>
            </DialogTitle>
          </DialogHeader>
          {calendarRecord && (
            <CycleCalendar
              cycleData={{
                firstPeriodDate: new Date(calendarRecord.last_period_date),
                cycleLength: calendarRecord.cycle_length,
                periodDuration: calendarRecord.period_length,
                initialRating: {
                  mood: calendarRecord.mood ?? 3,
                  libido: calendarRecord.libido ?? 3,
                  stress: calendarRecord.stress ?? 3,
                  sleep: calendarRecord.sleep_hours ?? 8,
                  energy: calendarRecord.energy ?? 3,
                }
              }}
              ratings={new Map()}
              onUpdateRating={() => { }}
              cycleId={calendarRecord.id}
            />
          )}
        </DialogContent>
      </Dialog>
