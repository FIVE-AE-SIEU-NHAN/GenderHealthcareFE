...
export default function DoctorListDashboard() {
  ...
  const allDoctorColumns = useMemo(() => [
    {
      ...
      render: (_doctor: DoctorProfile, index: number) => (
        <Badge variant="outline" className="font-mono bg-sky-400/15 border-sky-600">
    ...
    },
    {
      ...
      render: (doctor: DoctorProfile) => {
        ...
        return (
          <Badge className={cn("font-medium text-xs", specialization?.style)}>
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(allDoctorColumns.filter(col => col.defaultVisible !== false).map((col) => col.key));
  ...
  const renderDoctorActions = useCallback((doctor: DoctorProfile) => {
    const statusString = DOCTOR_STATUS.UI_MAP[doctor.status] || 'Unknown';
    const isMutatingThisDoctor =
      editDoctorStatusMutation.isPending &&
      editDoctorStatusMutation.variables?.doctorId === doctor.id;
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild disabled={isMutatingThisDoctor}>
          <Button variant="ghost" className="h-8 w-8 p-0">
            ...
            {isMutatingThisDoctor ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoreHorizontal className="h-4 w-4" />}
          </Button>
        </DropdownMenuTrigger>
  ...
  return (
    <>
      ...
      <Dialog 
        open={!!editingDoctor} 
        onOpenChange={(isOpen) => !isOpen && setEditingDoctor(null)}
      >
        <DialogContent className="sm:max-w-none w-[95vw] md:w-[60vw] lg:w-[50vw] xl:w-[40vw] flex flex-col p-0">
          <DialogHeader className="rounded-t-lg border-b p-6 pb-4 bg-gradient-to-br from-slate-50 to-slate-100">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center shrink-0">
