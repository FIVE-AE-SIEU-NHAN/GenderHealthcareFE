...
import { CONSULTANT_SEARCH_FIELDS, CONSULTANT_STATUS } from '@/Application/constants/manager/manager.consultantConstants';
import { BackendConsultantResponse, EditConsultantStatusPayload, EditConsultantStatusResponse, PaginatedConsultantsResponse, UpdateConsultantProfilePayload, UpdateConsultantProfileResponse, UseConsultantsOptions,  } from '@/types/manager/consultantTypes';
import { format } from 'date-fns';

// =============== Consultant FETCHING ===============
export const fetchConsultants = async ({ page, limit, filters, search, sort, dateRange }: UseConsultantsOptions): Promise<PaginatedConsultantsResponse> => {
...
export const editConsultantStatusAPI = async ({ consultantId, status }: EditConsultantStatusPayload): Promise<EditConsultantStatusResponse> => {
  const body = { status };
