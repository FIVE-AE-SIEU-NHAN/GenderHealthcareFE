...
import { BackendDoctorResponse, EditDoctorStatusPayload, EditDoctorStatusResponse, PaginatedDoctorsResponse, UpdateDoctorProfilePayload, UpdateDoctorProfileResponse, UseDoctorsOptions } from '@/types/manager/doctorTypes';
import { format } from 'date-fns';

// =============== Doctor FETCHING ===============
export const fetchDoctors = async ({ page, limit, filters, search, sort, dateRange }: UseDoctorsOptions): Promise<PaginatedDoctorsResponse> => {
...
export const editDoctorStatusAPI = async ({ doctorId, status }: EditDoctorStatusPayload): Promise<EditDoctorStatusResponse> => {
  const body = { status };
...
export const updateDoctorProfileAPI = async ({ 
  ...
  const response = await api.patch<UpdateDoctorProfileResponse>(
    `/staff/${doctorId}/update-profile`, 
    updateData
  );
  return response.data;
};