import React from "react";

import { Link } from 'react-router-dom';

import { Button } from "@/components/ui/button";

import { FaFacebook } from "react-icons/fa";
import { FaXTwitter } from "react-icons/fa6";
import { LuInstagram } from "react-icons/lu";

const ContactPage: React.FC = () => {
  return (
    <div className="font-sant bg-white relative h-170">
      
      {/* Extended blue background behind content */}
      <div className="absolute top-0 w-full h-80 bg-[#6E7BF2] rounded-b-[40px] z-0"></div>

      {/* Main content */}
      <div className="relative z-10 max-w-6xl mx-auto pt-20 grid grid-cols-1 md:grid-cols-2 gap-6 px-6 pb-12">
        {/* Left: Offices */}
        <div className="bg-white rounded-2xl shadow-md p-6">
          <h2 className="text-2xl font-bold mb-6 text-[#1C2359]">Offices</h2>

          <div className="mb-4">
            <h3 className="font-semibold text-[#1C2359]">Ha Noi</h3>
            <p className="text-sm text-gray-600">123, abc xyz, acbbca, xxxxxx</p>
          </div>

          <div className="mb-4">
            <h3 className="font-semibold text-[#1C2359]">Ho Chi Minh</h3>
            <p className="text-sm text-gray-600">123, abc xyz, acbbca, xxxxxx</p>
          </div>

          <div className="mb-6">
            <h3 className="font-semibold text-[#1C2359]">Can Tho</h3>
            <p className="text-sm text-gray-600">123, abc xyz, acbbca, xxxxxx</p>
          </div>

          <div className="flex space-x-4 text-xl">
            {/* <a href="#" className="text-blue-600"><i></i><FaFacebook /></a>
            <a href="#" className="text-blue-600"><i ></i><LuInstagram /></a>
            <a href="#" className="text-blue-600"><i></i><FaXTwitter /></a> */}
            <Link to={"https://www.facebook.com/"} className="text-blue-600" >
              <FaFacebook size={25} />
            </Link>
            <Link to={"https://x.com/"} className="text-blue-600" >
              <FaXTwitter size={25} />
            </Link>
            <Link to={"https://www.instagram.com/"} className="text-blue-600" >
              <LuInstagram size={25} />
            </Link>
          </div>
        </div>

        {/* Right: Contact Form */}
        <div className="bg-white rounded-2xl shadow-md p-6">
          <h2 className="text-2xl font-bold mb-6 text-[#1C2359]">Contact Us</h2>
          <form className="space-y-4">
            <div className="flex gap-4">
              <input
                type="text"
                placeholder="Enter your name"
                className="w-1/2 border-b border-gray-400 focus:outline-none text-sm"
              />
              <input
                type="email"
                placeholder="Enter a valid email address"
                className="w-1/2 border-b border-gray-400 focus:outline-none text-sm"
              />
            </div>

            <input
              type="tel"
              placeholder="Enter your phone (e.g +84 123 321 999)"
              className="w-full border-b border-gray-400 focus:outline-none text-sm"
            />

            <textarea
              placeholder="Message"
              className="w-full border-b border-gray-400 focus:outline-none resize-none h-24 text-sm"
            ></textarea>

            <div className="flex items-center text-sm">
              <input type="checkbox" id="terms" className="mr-2 accent-blue-500" />
              <label htmlFor="terms">
                I accept the <Link to={"#"} className="text-blue-600 underline">Terms of Servies</Link>
              </label>
            </div>

              <Link to={"#"}>
                  <Button className="mx-auto block w-1/2 bg-[#1A3973] text-white font-semibold rounded-md">
                    SEND MESSAGE
                  </Button>
              </Link>

            
          </form>
        </div>
      </div>
    </div>
  );
}

export default ContactPage;